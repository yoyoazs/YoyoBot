...
function processCommand(msg) {
    ...
    switch (musicCmd) {
        ...
        default:
            msg.channel.send(`Merci de vous référer a ${tool.wrap('~help music')}.`);
    }
}
    ...
    ySearch(searchQuery, opts, function (err, results) {
        ...
        ytdl.getInfo(results[y].link, function (err, song) {
            ...
            guild.queueSong(new Song(song.title, song.video_url, 'youtube',  author, time(song.length_seconds), song.iurlmq));
                    msg.channel.send(
            `Mise en file d'attente ${tool.wrap(song.title.trim())} (\`${time(song.length_seconds)}\`) envoyé par ${tool.wrap(author)}`
        );
    ...
    */
    song(msg, guild, url) {
        ytdl.getInfo(url, (err, song) => {
            if (err) {
                console.log(err);
                msg.channel.send(`Désolé, je ne peut pas ajouté votre musique a la liste.`);
                return;
            }
            const author  = msg.author.username + '#' + msg.author.discriminator;
            console.log(song);
            ...
            msg.channel.send(
                `Mise a la file d'atyente ${tool.wrap(song.title.trim())} (\`${time(song.length_seconds)}\`) envoyé par ${tool.wrap(author)}`
            );
            if (guild.status != 'playing') {
                guild.playSong(msg);
            }
    ...
    */
    playlist(msg, guild, playlistId) {
        ...
        async function addToQueue(playlistTitle, playlistItems) {
            ...
            msg.channel.send(
                `Ajoute a la file d'attente de ${tool.wrap(playlistItems.length)} chanson de ${tool.wrap(playlistTitle)} envoyé par ${tool.wrap(msg.author.username + '#' + msg.author.discriminator)}`
            );
    ...
    for (let guildId in guilds) {
        ...
        if (guild.inactivityTimer <= 0) {
            ...
            guild.musicChannel.send(
                ':no_entry_sign: Je viens de quitter le canal pour inactivité.');
