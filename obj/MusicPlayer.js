...
class MusicPlayer {
            ...
            resolveVoiceChannel.call(this).then(() => {
                ...
                this.musicChannel.send({embed: {
                    ...
                    fields: [{
                        ...
                        value: `[${song.title}]`+`(${song.url}) (\`${song.time}\`) envoyé par **${song.author}**`
                      }],
                    thumbnail: {
                        url: `${song.thumbnail}`
                    },
        ...
        function resolveVoiceChannel() {
            return new Promise((resolve, reject) => {
                if (this.voiceConnection)
                    ...
                    msg.channel.send(
                        `Je ne suis pas dans un channel vocal faite ${tool.wrap('y/music join')} pour commencer à jouer la liste de musique.`
                    );
                    reject('novoice');
                }
            });
    ...
    */
    skipSong() {
        if (this.dispatch && this.status == 'playing') {
            this.musicChannel.send(`:fast_forward: Skip ${tool.wrap(this.queue[0].title)}`);
            this.dispatch.end();
        } else {
            this.musicChannel.send(`Il n'y a rien a skip!`);
        }
    }
        ...
        if (this.queue.length > 0) {
            try {
                ...
                for (let i = 0; i < this.queue.length && i < 15; i++)
                    queueString += `${i + 1}. ${this.queue[i].title} (\`${this.queue[i].time}\`) envoyé par ${this.queue[i].author}\n`;
                ...
                msg.channel.send(
                    `${tool.inaError} Je ne peut pas afficher la liste, réesayer plus tard.`
                );
            }
        } else {
            msg.channel.send(`Il n'y a auqu'une musique dans la liste!`);
        }
    }
    ...
    */
    purgeQueue(msg) {
        this.queue = [];
        msg.channel.send('La liste a étais clear.');
    }
    ...
    nowPlaying(msg) {
        if (this.queue.length > 0){
            msg.channel.send({embed: {
                    ...
                    fields: [{
                        name: `:notes: Lecture en cours `,
                        value: `[${this.queue[0].title}]`+`(${this.queue[0].url}) (\`${this.queue[0].time}\`) envoyé par **${this.queue[0].author}**`
                      }],
                    thumbnail: {
    ...
    setVolume(msg) {
        ...
        if (vol && (vol >= 0 && vol <= 1)) {
            if (this.dispatch) {
                ...
                this.volume = vol;
                msg.channel.send(`:speaker:Le volume a étais définit a ${tool.wrap(vol * 100)}`);
            } else {
                msg.channel.send("Rien n'est joué en ce moment.");
            }
        } else {
        ...
        if (msg.member.voiceChannel) {
            ...
            msg.member.voiceChannel.join().then(connection => {
                ...
                this.musicChannel.send(
                    `Rejoin et lié a :speaker: **${msg.member.voiceChannel.name}** et #**${this.musicChannel.name}**.`
                );
                this.changeStatus('stopped');
                if (this.queue.length > 0)
                    this.playSong(msg);
            ...
            msg.channel.send(`Vous n'êtes pas en vocal!`);
        }
    }
    ...
    */
    leaveVc(msg) {
        if (this.voiceConnection) {
            ...
            msg.channel.send(`Je ne suis pas en vocal!`);
        }
    }
